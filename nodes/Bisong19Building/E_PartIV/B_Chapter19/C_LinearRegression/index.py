...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
blocks = [
    mbk("In this example, we will implement a linear regression model with Scikit-learn. The model will predict house prices from the Boston house-prices dataset. The dataset contains 506 observations and 13 features."),
    mbk("""
...
Root mean squared error (RMSE): 4.33
    """),
    mbk("In the preceding code, using the train_test_split() function, the dataset is split into training and testing sets. The linear regression algorithm is applied to the training dataset to find the optimal values that parameterize the weights of the model. The model is evaluated by calling the .predict() function on the test set."),
    mbk("The error of the model is evaluated using the RMSE error metric (discussed in Chapter 14)."),
]
...
class Content(LeafNode):
    def __init__(self):
        ...
        self.add(mbk("# Linear Regression with Scikit-learn"))
