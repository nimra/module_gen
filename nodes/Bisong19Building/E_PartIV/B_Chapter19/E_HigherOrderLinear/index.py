...
Root mean squared error (RMSE): 3.01
    """),
    mbk("From the example, we can observe a slight improvement in the error score of the model with added higher-order features. This result is similar to what may most likely be observed in practice. It is rare to find datasets from real-world events where the features have a perfectly underlying linear structure. So adding higher-order terms is most likely to improve the model performance. But we must watch out to avoid overfitting the model."),
]
...
class Content(LeafNode):
    def __init__(self):
        ...
        self.add(mbk("# Higher-Order Linear Regression with Scikit-learn"))
