...
from .A_Shellor.index import Shellor as A_Shellor
from .B_Helpand.index import Helpand as B_Helpand
from .C_KeyboardShortcuts.index import KeyboardShortcuts as C_KeyboardShortcuts
from .D_IPythonMagic.index import IPythonMagic as D_IPythonMagic
from .E_Inputand.index import Inputand as E_Inputand
from .F_IPythonand.index import IPythonand as F_IPythonand
from .G_ShellRelatedMagic.index import ShellRelatedMagic as G_ShellRelatedMagic
from .H_Errorsand.index import Errorsand as H_Errorsand
from .I_Profilingand.index import Profilingand as I_Profilingand
from .J_MoreIPython.index import MoreIPython as J_MoreIPython
...
class Chapter1(HierNode):
    def __init__(self):
        super().__init__("Chapter 1. IPython: Beyond Normal Python")
        self.add(Content())
        self.add(A_Shellor())
        self.add(B_Helpand())
        self.add(C_KeyboardShortcuts())
        self.add(D_IPythonMagic())
        self.add(E_Inputand())
        self.add(F_IPythonand())
        self.add(G_ShellRelatedMagic())
        self.add(H_Errorsand())
        self.add(I_Profilingand())
        self.add(J_MoreIPython())
